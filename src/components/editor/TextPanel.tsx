...
import type { TimelineItem, TextProps, TextGradient } from "@/lib/editor";
import { DEFAULT_TEXT_PROPS, TEXT_STYLE_PRESETS, DEFAULT_TRANSFORM, DEFAULT_FILTERS, createDefaultItem } from "@/lib/editor";
import { withHistory } from "@/lib/editor/history";
import { Type, Plus, Captions, Play } from "lucide-react";
...
const TEXT_TEMPLATES: {
  ...
  {
    label: "Título",
    description: "Fonte grande em destaque",
    name: "Título",
    durationSeconds: 5,
    overrides: {
  ...
  {
    label: "Subtítulo",
    description: "Fundo escuro inferior legível",
    name: "Subtítulo",
    durationSeconds: 5,
    overrides: {
      content: "Subtítulo do vídeo",
      fontSize: 36,
      y: 82,
      textAlign: "center",
      color: "#ffffff",
  ...
  {
    label: "Balão de fala",
    description: "Balão estilizado com fundo",
    name: "Balão de fala",
    durationSeconds: 4,
    overrides: {
      content: "Olá! 👋",
...
export default function TextPanel({ onNavigate }: TextPanelProps) {
  ...
  const textItems = [...project.timeline.items]
    .filter((item: TimelineItem) => item.kind === "text")
    .sort((a: TimelineItem, b: TimelineItem) => a.startFrame - b.startFrame);
  ...
  const addTextItem = (
    ...
    const item = createDefaultItem({
      ...
      durationInFrames: Math.round(project.timeline.fps * durationSeconds),
      name,
      kind: "text",
      transform: { ...DEFAULT_TRANSFORM },
      filters: { ...DEFAULT_FILTERS },
      text: { ...DEFAULT_TEXT_PROPS, content: "Novo Texto", ...overrides },
    });
  ...
  const handleAddText = () => addTextItem();
  ...
  const formatFrameTime = (frame: number): string => {
    const totalSec = Math.max(0, frame) / project.timeline.fps;
    ...
    <div className="h-full bg-[#0d0d16] border-r border-[#1e1e2e] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-4">
        ...
        <section>
          <button
            onClick={handleAddText}
            className="w-full py-2.5 bg-[#8b5cf6] hover:bg-[#7c3aed] rounded-lg text-white text-xs font-semibold flex items-center justify-center gap-2 transition-colors"
          >
            <Plus size={14} />
            Adicionar Texto Padrão
          </button>
        ...
        <section>
          <h4 className="text-[10px] text-gray-500 uppercase tracking-wider mb-2">Modelos</h4>
          <div className="grid grid-cols-1 gap-1.5">
            {TEXT_TEMPLATES.map((t) => (
              <button
                key={t.label}
                onClick={() => addTextItem(t.overrides, t.name, t.durationSeconds)}
                className="w-full text-left px-3 py-2 rounded-lg border border-[#1e1e2e] bg-[#13131f] hover:border-[#8b5cf6] transition-colors flex items-center justify-between gap-2"
              >
                <span className="min-w-0">
                  <span className="block text-[11px] font-semibold text-gray-200">{t.label}</span>
                  <span className="block text-[9px] text-gray-500 truncate">{t.description}</span>
                </span>
                <Plus size={12} className="text-[#8b5cf6] flex-shrink-0" />
              </button>
            ))}
          </div>
        </section>
        ...
        <section>
          <h4 className="text-[10px] text-gray-500 uppercase tracking-wider mb-2">Legendas Automáticas</h4>
          <button
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-2 py-1.5 rounded border transition-colors ${
                    selectedIds.has(item.id)
                      ...
                      : "border-[#1e1e2e] bg-[#13131f] hover:border-[#8b5cf6]"
                  }`}
                >
                  <Play size={10} className="text-[#8b5cf6] flex-shrink-0" />
                  <span className="flex-1 text-left text-[11px] text-gray-300 truncate">
                    {item.text?.content || item.name || "Texto"}
                  </span>
                  <span className="text-[9px] text-gray-500 flex-shrink-0">
                    {formatFrameTime(item.startFrame)}
                  </span>
                </button>
              ))}
            </div>
        ...
        {!selectedTextItem ? (
          <div className="text-center text-gray-600 text-[11px] py-6">
            Selecione um texto acima para editar detalhes, conteúdo e estilo.
          </div>
        ) : (
          <>
            {/* Conteúdo */}
            ...
            <section>
              ...
              <div className="space-y-2">
                <div>
                  ...
                  />
                  <button
                    onClick={() => handleUpdateText({ strokeEnabled: !text.strokeEnabled })}
                    className={`mt-1 w-full py-1 text-[10px] rounded border transition-colors ${
                      text.strokeEnabled
                        ? "bg-[#8b5cf6] border-[#8b5cf6] text-white"
                        : "bg-[#13131f] border-[#1e1e2e] text-gray-400 hover:border-[#8b5cf6]"
                    }`}
                  >
                    {text.strokeEnabled ? "Contorno Ativo" : "Ativar Contorno"}
                  </button>
                </div>
